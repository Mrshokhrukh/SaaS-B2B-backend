...
@Injectable()
export class MockClickProvider implements PaymentProviderAdapter {
  ...
  createIntent(input: {
    ...
    const providerPaymentId = `click_${randomUUID()}`;

    return Promise.resolve({
      providerPaymentId,
      checkoutUrl: `https://checkout.mock-click.local/pay/${providerPaymentId}`,
      payload: {
  ...
  verifySignature(payload: WebhookDto, signature: string): boolean {
    const secret = this.configService.get<string>('payments.clickSecret') ?? '';
